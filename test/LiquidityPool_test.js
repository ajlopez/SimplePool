...
contract('LiquidityPool', function (accounts) {   
    ...
    describe('deposits', function () {
        beforeEach(async function () {
            poolToken = await BasicToken.new('Token', 'PTK');
            token = await BasicToken.new('Token', 'TOK');
            await token.transfer(alice, 1000000);
            liquidityPool = await LiquidityPool.new(poolToken.address, token.address);
        });
        
        it('initial state', async function () {
            const initialToken = await liquidityPool.token();
            const initialPoolToken = await liquidityPool.poolToken();
            const tokenBalance = Number(await liquidityPool.tokenBalance());
            const cryptoBalance = Number(await liquidityPool.cryptoBalance());
            
            assert.equal(initialToken, token.address);
            assert.equal(initialPoolToken, poolToken.address);
